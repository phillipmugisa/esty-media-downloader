(() => {
    ...
    const sites = {
        "etsy.com" : {
            "productTitle" : ".wt-mb-xs-1 h1",
            "body": "body",
            "mainProdImage": ".carousel-pagination-item-v2 img",
            "variationImages": {
                "main" : ".carousel-pagination-item-v2 img"
            },
            "reviewImages": {
    ...
    const handleDownload = () => {
        ...
        if (app.getUserFeatures()) {        
            ...
            if (PRICING.toLocaleLowerCase() == "Free".toLocaleLowerCase()) {
                ...
                if (selectedFeatures.includes("review images")) {
    ...
    const downloadProductMain = () => {
        ...
        let domElem = sites[`${app.getSourceName()}`]['mainProdImage'];
        ...
        if (targetImages != null && targetImages != undefined) {
            targetImages.forEach(image => {
                ...
                let generateSrc = imageSrc.split("il_75x75").join("il_NxN")
                // to jpeg
                // download image
			try {
                performDownload(generateSrc, 'product-images', 'jpeg')
                // toJpeg(generateSrc)
                //     .then(url => {
                // });                    
        ...
        if (targetImages != null && targetImages != undefined) {
            targetImages.forEach(image => {
                ...
                if (!imageSrc.includes("gif")) {
                    ...
                    // to jpeg
                    // download image
                    performDownload(generateSrc, 'variation-imgs', 'jpeg')
                }
            });
        }
    }

    const downloadreviewImages = () => {
        let targetImages;
        let domElem = sites[`${app.getSourceName()}`]['reviewImages']['main'];
        targetImages = document.querySelectorAll(domElem);
        ...
        if (targetImages != null && targetImages != undefined) {
            targetImages.forEach(image => {
                ...
                if (!imageSrc.includes(".gif")) {
                    // get jpg src
                    let generateSrc = imageSrc.split("iap_75x75").join("iap_NxN")

                    // to jpeg
                    // download image
                    performDownload(generateSrc, 'review-images', 'jpeg')
                    // toJpeg(generateSrc)
                        // .then(url => performDownload(url, 'review-images', 'jpeg'));
                }
    ...
    const showGuideModal = () => {
        ...
        <main>
            <div class="review">
                <h3 id="review-title">Downloading Product Images</h3>
                <ul id="steps"></ul>
            </div>
    ...
    const renderModalCategory = (modalElem, categoryData) => {
        const review = modalElem.querySelector(".review");
        const modalImg = modalElem.querySelector(".modal-img");
        
        const reviewTitle = review.querySelector(".review #review-title");
        const reviewList = review.querySelector(".review #steps");
        ...
        reviewTitle.textContent = `Downloading ${categoryData["title"]}`;
            ...
            "aliexpress.com" : {
                categories: [
                    ...
                    },
                    {
                        title: "review Images",
                        data : [
                            "review Images are located in the position showed in the image.",
                            "Scroll to download of page to view desired review images.",
                            "Open/Reopen download popup and select review images option to download.",
                        ],
                        svg: aliExpressDesc
                    }
                ]
                ...
                categories: [
                    ...
                    },
                    {
                        ...
                        data : [
                            "review Images are located in the position showed in the image.",
                            "Scroll to download of page to view desired review images.",
                            "Some products donot contain review images.",
                            "Open/Reopen download popup and select review images option to download.",
                        ],
                        svg: amazonDesc
                    }
